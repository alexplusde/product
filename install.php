...
if (rex_addon::get('yform') && rex_addon::get('yform')->isAvailable()) {
    rex_yform_manager_table_api::importTablesets(rex_file::get(__DIR__ .  'install/rex_product_tableset.json'));
}

if (!rex_media::get('product_fallback_image.png')) {

    rex_file::copy(__DIR__ . '/install/product_fallback_image.png', rex_path::media('product_fallback_image.png'));
    $data = [];
...
if (rex_addon::get('url') && rex_addon::get('url')->isAvailable()) {
    ...
    if (false === rex_config::get('product', 'url_profile', false)) {
        ...
        if (!$rex_product_category) {
            $query = rex_file::get(__DIR__ .  'install/rex_url_profile_product_category.sql');
        ...
        if (!$rex_product) {
            $query = rex_file::get(__DIR__ . 'install/rex_url_profile_product.sql');
            rex_sql::factory()->setQuery($query);
        }
