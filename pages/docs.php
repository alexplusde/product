...
$mdFiles = [];
foreach (glob(rex_addon::get('product')->getPath('docs') . '/*.md') ?: [] as $file) {
...
$page = rex_be_controller::getPageObject('product/docs');
...
if (null !== $page) {
    foreach ($mdFiles as $key => $mdFile) {
        ...
        $page->addSubpage(
            (new rex_be_page($key, rex_i18n::msg('product_docs_' . $keyWithoudPrio)))
            ->setSubPath($mdFile)
            ->setHref('index.php?page=product/docs&mdfile=' . $key)
